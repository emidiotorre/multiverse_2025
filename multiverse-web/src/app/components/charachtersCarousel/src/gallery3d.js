...
// --- REALIGN TILT -----------------------------------------------------------
const REALIGN_MS_DESKTOP = Math.round(MOVE_MS * 0.6);
const REALIGN_MS_MOBILE = Math.round(MOBILE_TIMING.MOVE_TO_SIDE_MS * 0.7);
...
function getDynamicSpacing() {
  const t = clamp(
    (window.innerWidth - CAMERA_CONFIG.MOBILE_MAX) /
      (1920 - CAMERA_CONFIG.MOBILE_MAX),
    0,
    1,
  );
  return 1.0 + (1.5 - 1.0) * t;
}
function snapTransform(obj, pos, scale) {
...
// ENTRY POINT
// ============================================================================
export async function initGallery3D(canvasRefOrSelector) {
  // Canvas + Renderer
  // Support both React ref and CSS selector
  const canvas =
    canvasRefOrSelector?.current ||
    (typeof canvasRefOrSelector === "string"
      ? document.querySelector(canvasRefOrSelector)
      : canvasRefOrSelector);
  if (!canvas)
    throw new Error(
      "Canvas non trovato: " +
        (typeof canvasRefOrSelector === "string" ? canvasRefOrSelector : "ref"),
    );

  // Debug logging for canvas initialization
  console.log("Gallery3D: Initializing with canvas:", {
    element: canvas,
    id: canvas.id,
  ...
  function isMobile() {
    // Use canvas container width for more accurate mobile detection
    ...
    const containerWidth =
      ...
      canvas.offsetWidth ||
      window.innerWidth;
    return containerWidth <= CAMERA_CONFIG.MOBILE_MAX;
  }
  ...
  function setupLights() {
    ...
    key.position.set(...LIGHTS.key.pos);
    scene.add(key);
    rim = new THREE.DirectionalLight(LIGHTS.rim.color, LIGHTS.rim.intensity);
    rim.position.set(...LIGHTS.rim.pos);
    scene.add(rim);
    fill = new THREE.DirectionalLight(LIGHTS.fill.color, LIGHTS.fill.intensity);
    fill.position.set(...LIGHTS.fill.pos);
    scene.add(fill);
  }
  ...
  // UI - Create overlay div instead of replacing container innerHTML
  const container = document.getElementById("characters-carousel-container");

  // Create UI overlay div
  const uiOverlay = document.createElement("div");
  uiOverlay.className = "ui";
  uiOverlay.innerHTML = `
    <div class="topbar">
  ...
  // Append UI overlay to container (preserving existing canvas)
  container.appendChild(uiOverlay);

  const chipName = uiOverlay.querySelector("#chipName");
  const chipRole = uiOverlay.querySelector("#chipRole");
  const descCard = SHOW_DESC ? uiOverlay.querySelector("#descCard") : null;
  const btnLeft = uiOverlay.querySelector("#navLeft");
  const btnRight = uiOverlay.querySelector("#navRight");
  ...
  async function loadModel(entry) {
    ...
    const meta = {
      ...
      role: entry.role ?? "—",
      desc: Array.isArray(entry.phrases)
        ? entry.phrases.join(" ")
        : (entry.desc ?? ""),
    };
  ...
  function getViewportConfig() {
    if (isMobile())
      return {
        spacing: SPACING_DESKTOP_BASE * 3.4,
        showSides: false,
        otherOpacity: 0.0,
      };
    return {
  ...
  function applySelectionMobile(index, first = false) {
    ...
    const offL = new THREE.Vector3(
      -spacing * MOBILE_GEOM.OFF_MULT,
      OFF_Y,
      backOffset * MOBILE_GEOM.OFF_Z_MULT,
    );
    const offR = new THREE.Vector3(
      +spacing * MOBILE_GEOM.OFF_MULT,
      OFF_Y,
      backOffset * MOBILE_GEOM.OFF_Z_MULT,
    );
    ...
    tweenPos(
      ...
      () => {
        ...
        snapTransform(
          outgoing,
          outOff,
          outgoing.userData.baseScale * OTHER_SCALE,
        );
    ...
    snapTransform(
      incoming,
      inStartVec,
      incoming.userData.baseScale * OTHER_SCALE,
    );
    ...
    tweenScale(
      incoming,
      sBase,
      MOBILE_TIMING.MOVE_TO_SIDE_MS * 0.45,
      MOBILE_TIMING.MOVE_TO_SIDE_MS * 0.55,
    );
    tweenOpacities(
      incoming,
      1.0,
      0.0,
      CROSSFADE_MS,
      MOBILE_TIMING.IN_FADE_DELAY,
    );

    cancelTweens(incoming, "roty");
    cancelTweens(incoming, "rotxz");
  ...
  function applySelectionDesktop(index, first = false) {
    ...
    roots.forEach((r, i) => {
      ...
      if (newState === "center") {
        targetPos = center;
        targetScale = r.userData.baseScale * SELECT_SCALE;
        spin = 0;
      } else if (newState === "sideL") {
        targetPos = left;
        targetScale = r.userData.baseScale * OTHER_SCALE;
        spin = BASE_SPIN * 0.6;
      } else if (newState === "sideR") {
        targetPos = right;
        targetScale = r.userData.baseScale * OTHER_SCALE;
        spin = BASE_SPIN * 0.6;
      } else if (newState === "offL") {
        targetPos = offL;
        targetScale = r.userData.baseScale * OTHER_SCALE;
        spin = 0;
      } else {
      ...
      if (newState === "center") {
        r.visible = true;
        if (!first)
          tweenOpacities(r, 1.0, 0.0, CROSSFADE_MS, CROSSFADE_DELAY_MS);
        ...
        if (!first && (prevState === "offL" || prevState === "offR")) {
          cancelTweens(r);
          if (newState === "sideL") r.position.copy(offL);
          else r.position.copy(offR);
      ...
      if (spin === 0) cancelSpin(r);
        ...
        r.userData.isFrozen = false;
        setSpinTarget(r, spin);
      }
  ...
  function exitWithSilhouette(
    r,
    fadeMs = FADE_OUT_MS,
    fadeDelay = FADE_OUT_DELAY_MS,
    keepSilMs = PHASE_KEEP_SIL_MS,
    onDone,
  ) {
    ...
    for (const a of Array.from(anims)) {
      ...
      if (a.type === "roty") a.obj.rotation.y = a.y0 + (a.y1 - a.y0) * e;
        ...
        for (let i = 0; i < matsO.length; i++) {
          matsO[i].opacity = startO[i] + (endO - startO[i]) * e;
        }
        for (let i = 0; i < matsS.length; i++) {
          matsS[i].opacity = startS[i] + (endS - startS[i]) * e;
        }
      }
  ...
  // Auto-spin
  function setSpinTarget(obj, v) {
    obj.userData.rotTarget = v;
  }
  function cancelSpin(obj) {
    obj.userData.rotTarget = 0;
    obj.userData.rotSpeed = 0;
    obj.userData.isFrozen = true;
  }
  ...
  function endDrag(e) {
    ...
    if (r) {
      r.userData.isFrozen = false;
      r.userData.rotSpeed = drag.vx;
      r.userData.rotTarget = 0;
    }
    if (drag.id != null) {
      try {
  ...
  function next() {
    navDir = +1;
    endDrag({ pointerId: drag.id });
    lastSelectedIndex = selectedIndex;
    selectedIndex = (selectedIndex + 1) % roots.length;
  ...
  function prev() {
    ...
    endDrag({ pointerId: drag.id });
    lastSelectedIndex = selectedIndex;
    selectedIndex = (selectedIndex - 1 + roots.length) % roots.length;
    applySelection(selectedIndex);
  }
  ...
  console.log("Gallery3D: Canvas before prewarming:", {
    visibility: canvas.style.visibility,
    display: canvas.style.display,
  ...
  console.log("Gallery3D: Canvas after prewarming:", {
    visibility: canvas.style.visibility,
    display: canvas.style.display,
    dimensions: canvas.getBoundingClientRect(),
  });
  ...
  applySelection(selectedIndex, true);
  console.log("Gallery3D: Starting render loop");
  requestAnimationFrame(tick);
  ...
  function resizeRenderer() {
    // Get canvas container dimensions instead of window dimensions
    const rect = canvas.getBoundingClientRect();
    const w = rect.width || canvas.clientWidth || canvas.offsetWidth;
    const h = rect.height || canvas.clientHeight || canvas.offsetHeight;

    // Fallback to window dimensions if container has no size
    const finalW = w > 0 ? w : window.innerWidth;
    const finalH = h > 0 ? h : window.innerHeight;

    console.log("Gallery3D: Resizing renderer:", {
      containerRect: rect,
      calculatedSize: { w, h },
      finalSize: { finalW, finalH },
      windowSize: { width: window.innerWidth, height: window.innerHeight },
      isMobile: finalW <= CAMERA_CONFIG.MOBILE_MAX,
    });

    const dprCap = finalW <= CAMERA_CONFIG.MOBILE_MAX ? 1.5 : 1.75;
    const dpr = Math.min(window.devicePixelRatio || 1, dprCap);
    renderer.setPixelRatio(dpr);
    renderer.setSize(finalW, finalH, true);
